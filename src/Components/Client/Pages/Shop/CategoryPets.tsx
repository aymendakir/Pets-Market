...
const CategoryPetsShop = ({value,handleChoose}:CategroyProps) => {
  const {  FetchPets } = useContext(ApiContext);

  const { data: ListPets } = useQuery(["ListPetsShop"], FetchPets, {
    ...
    <main className="mt-20 w-[90%] mx-auto min-h-[60vh] space-y-12">
      ...
      >
        {ListPets?.success&&ListPets.response.map((pet,index)=> (
            <div key={index} className="h-[250px] !w-[200px]    mb-4" onClick={()=>{
              handleChoose(pet.Pets_name)
            }}>
              ...
              <p className="font-bold text-[20px] mt-5  text-center">{pet.Pets_name}</p>
            </div>
